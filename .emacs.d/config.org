...
#+begin_src emacs-lisp
  ...
  (defun log-message (beg end)
    ;; Do something with the response between beg and end
    (async-shell-command "log_message"))
  ...
  (use-package gptel
    ...
    (require 'auth-source)
    (add-hook 'gptel-post-response-functions 'log-message)

...
#+begin_src emacs-lisp
  ...
  ;; write another tool in this file that will read the contents of a url
  (gptel-make-tool
   :name "read_url"
   :function (lambda (url)
               (with-temp-buffer
  ...
  ;; @qq elisp code to enable the above tools globally

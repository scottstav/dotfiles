...
#+begin_src emacs-lisp
  (use-package mu4e
    ...
    ;; Split headers (left) | article (right)
    ...
    ;; Bookmarks
    (setq mu4e-bookmarks
          '((:name "Unread"    :query "flag:unread AND maildir:/gmail/Inbox" :key ?u)
...
#+begin_src emacs-lisp
  (use-package mu4e-alert
    ...
    ;; Notify directly via D-Bus — one notification per email, click to open
    ...
    (defun my/mu4e-alert-notify (mails)
      "Send one D-Bus notification per unread mail with click-to-open."
      (dolist (mail (seq-take mails 10))
        (let* ((sender (let ((from (car (plist-get mail :from))))
                         (or (plist-get from :name) (plist-get from :email) "Unknown")))
               (subject (or (plist-get mail :subject) "(no subject)"))
               (msgid (plist-get mail :message-id))
  ...
  ;; Disable mu4e's built-in modeline indicator (U:n/n)

...
description = """Add or update a field on a contact, or create a new contact.
...
Birthday accepts: 'YYYY-MM-DD', 'MM-DD', 'Month Day', 'Month Day, Year' (e.g. 'December 16' or 'December 16, 1990').
...
def run(input):
    ...
    create_new = input.get("create_new", False)

    vcard_field = FIELD_MAP[field]

    # Parse/normalize value
    if field == "birthday":
        value = _parse_birthday(raw_value)
        if not value:
            return (
        ...
        value = f";;{raw_value.replace(chr(10), ' ')};;;;"
    else:
        value = raw_value
    ...
    if vcf_path is None:
        ...
        vcard_text = _create_vcard(contact_name, vcard_field, value, label or None)
        vcf_path.write_text(vcard_text)
        return (
            f"Created new contact '{contact_name}' with {field}: {raw_value}\n"
    ...
    modified, changed = _update_vcard(text, vcard_field, value, label or None)

    if not changed:
        return f"'{display_name}' already has {field} set to '{raw_value}'. No changes made."

    vcf_path.write_text(modified)

    result = f"Updated '{display_name}': set {field} to '{raw_value}'"
    if alternatives:
